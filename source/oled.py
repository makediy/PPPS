...
from hanzi import hz16,hz32,hz24
Font_6_8 = const(1)
Font_8_16 = const(2)
Font_12_24 = const(3)
Font_16_32 = const(4)
...
class OLED12864_I2C():
    ...
    def HZ16(self,x,y,n):
        self.set_pos(x,y)
        self.i2c.writeto(self.ADDR,bytearray(b'\x40'+hz16[n*2]))
        self.set_pos(x,y+1)   
        self.i2c.writeto(self.ADDR, bytearray(b'\x40'+hz16[n*2+1]))
    def HZ32(self,x,y,n):
        for i in range(4):
            self.set_pos(x,y+i)
            self.i2c.writeto(self.ADDR,bytearray(b'\x40'+hz32[n*4+i]))
    def HZ24(self,x,y,n):
        for i in range(3): 
            self.set_pos(x,y+i)
            self.i2c.writeto(self.ADDR,bytearray(b'\x40'+hz24[n*3+i]))